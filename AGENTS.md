...
в затрагиваемой части репозитория, а не альтернативу. Если альтернатива
кажется более удачной — не подставляйте её молча: спросите пользователя,
объясните причину и дождитесь подтверждения.

Если изменение затрагивает структуру папок или стек технологий (новый/удалённый проект, новая зависимость, обновление версии, которое стоит зафиксировать, новая конвенция) — обновляйте этот файл в рамках того же изменения, а не отдельным заходом позже.

Перед изменением существующих тестов или написанием новых сначала спросите пользователя — уточните, что и как должно быть покрыто (или что изменение достаточно тривиально и уточнение не требуется), а не решайте это самостоятельно.

Сообщения коммитов оформляются по Conventional Commits (`<тип>(<область>): <описание>`, например `feat(manifest): ...`, `fix(...): ...`, `docs(agents): ...`, `test(...): ...`, `chore(...): ...`, `refactor(...): ...`) - область необязательна, но желательна, если уточняет, что именно изменилось.

Не делайте `git push` - коммитьте локально, а пуш оставляйте пользователю, если только он явно не попросит запушить.
