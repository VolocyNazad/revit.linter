...
public abstract partial class RevitInteractionViewModel : InitializableObservableObject
{
    ...
    private async void FamilyLoadedIntoDocument(object? sender, FamilyLoadedIntoDocumentEventArgs e) => OnRevitChanged(RevitEventType.FamilyLoadedIntoDocument);
    private async void DocumentChanged(object? sender, DocumentChangedEventArgs e) => OnRevitChanged(RevitEventType.DocumentChanged);
    private async void ViewActivated(object? sender, ViewActivatedEventArgs e) => OnRevitChanged(RevitEventType.ViewActivated);
    private async void DocumentCreated(object? sender, DocumentCreatedEventArgs e) => OnRevitChanged(RevitEventType.DocumentCreated);
    private async void DocumentOpened(object? sender, DocumentOpenedEventArgs e) => OnRevitChanged(RevitEventType.DocumentOpened);
    private async void DocumentClosed(object? sender, DocumentClosedEventArgs e) => OnRevitChanged(RevitEventType.DocumentClosed);
